...
export default function UseCasesEducatorsPage() {
  const { t } = useTranslation();

  const features = [
    t('use_case_educators.features_1'),
    t('use_case_educators.features_2'),
    t('use_case_educators.features_3'),
    t('use_case_educators.features_4'),
    t('use_case_educators.features_5'),
  ];

  return (
    <div className="max-w-3xl mx-auto px-6 py-20">
      <p className="text-sm font-semibold uppercase tracking-widest text-amber-500 mb-2">{t('nav.use_cases')}</p>
      <h1 className="font-serif text-4xl md:text-5xl font-bold text-amber-900">{t('use_case_educators.title')}</h1>
      <p className="mt-4 text-amber-700 text-lg leading-relaxed">
        {t('use_case_educators.desc')}
      </p>
